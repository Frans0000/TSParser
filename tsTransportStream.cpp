...
int32_t xTS_PacketHeader::Parse(const uint8_t* Input)
{
	...
	m_SB = (Head & 0b11111111000000000000000000000000) >> 24;
...
int32_t xPES_PacketHeader::Parse(const uint8_t* Input, uint32_t DataLength)
{
	...
	// check whether stream_id indicates other additional fields
...
int32_t xPES_PacketHeader::getHeaderLength() const
{
	if (m_StreamId == eStreamId_program_stream_map ||
		...
		m_StreamId == eStreamId_DSMCC_stream ||
		m_StreamId == eStreamId_ITUT_H222_1_type_E) {
		return 6; // only basic fields
	}

	return 9 + m_PES_header_data_length; // 6 + 3 + length of additional fields
}
...
{
	m_PID = PID;
	m_BufferSize = 65536; // 64KB for a packet cuz 2^16
...
void xPES_Assembler::xBufferAppend(const uint8_t* Data, int32_t Size)
{
	if (m_DataOffset + Size <= m_BufferSize) {
		...
		if (m_OutputFile && Size > 0) {
			...
			fflush(m_OutputFile);  // write to disc
		}
	}
}
...
{
	int32_t offset = xTS::TS_HeaderLength; // 4 bytes of TS header

	if (PacketHeader->hasAdaptationField()) {
		offset += 1 + AdaptationField->getAdaptationFieldLength(); //1 byte of length + AF
	}
	return offset;
}
	...
	const xTS_AdaptationField* AdaptationField)
{
	...
	// calculating offset for payload
	...
	if (PacketHeader->getS()) { // Payload Unit Start Indicator = new PES packet

		// new PES packet - reset all
		m_PESH.Reset();
	...
	else { // continue with PES packet

		// check whether started and CC
		if (m_Started) {
